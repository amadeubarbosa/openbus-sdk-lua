...
local function tcall_cont(assistant, kind, func, args, ok, ...)
  ...
  callobserver(assistant, kind, ..., args())
...
local function onInvalidLogin(conn)
  ...
  while assistant.loginargs ~= nil and assistant.logouttask == nil do
...
function SetPropTask:receive()
  ...
  while not offer:invalid() do
    ...
    callobserver(self, "SetOfferProps", result, offer, properties)
  end
end
...
local Assistant = class{ interval = 5 }

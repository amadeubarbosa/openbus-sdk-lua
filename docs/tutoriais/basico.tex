...
\title{Tutorial do SDK Lua do OpenBus}
\author{Tecgraf \\
...
\begin{itemize}
  \item Entrar no diretório \$OPENBUS\_HOME/demo/lua/hello.
  \item Editar o arquivo Hello.properties em \$OPENBUS\_HOME/demo/lua/hello e configurar a localização do barramento.
...
Caso seja obtido algum erro, pode não ser fácil identificar a causa corretamente para quem não está acostumado ainda ao uso do barramento. No entanto, é bastante comum que o usuário esqueça de algum dos passos iniciais que devem ser informados pelo administrador do barramento, tais como:
\begin{itemize}
  \item Definir a variável de ambiente OPENBUS\_HOME .
  \item Carregar as configurações do barramento no \emph{shell}.
  \item Passar pelo processo de implantação, que incluio cadastro e autorização, e também pode incluir a geração de certificados.
\end{itemize}

Para descrever melhor esses passos, mantemos uma página com explicações em \url{https://jira.tecgraf.puc-rio.br/confluence/display/OpenBus/Como+instalar+o+SDK+Lua+e+o+executar+o+HelloService+(v+1.5.2)}.

\section{Desenvolvimento}

\subsection{API}\label{api}

A API é composta pelo módulo \textit{openbus.Openbus}, pela interface \textit{IRegistryService} advinda da IDL do Serviço de Registro e pelo módulo \textit{faulttolerance.SmartComponent}. O módulo Openbus atualmente é implementado como o padrão \emph{singleton}, pois suporta apenas um barramento. Para se obter a instância do barramento e inicializá-la, deve-se utilizar o Código~\ref{lst:barramento}, podendo haver varições nos parâmetros passados ao método \emph{init}.
...
inicializa o Orb.
  Os parâmetros são:
  \begin{description}
    \item[host:] Endereço do Serviço de Controle de Acesso.
    \item[port:] Porta do Serviço de Controle de Acesso.
    \item[props:] Conjunto de propriedades para a criação do ORB.
    \item[serverInterceptorConfig:] Configuração opcional do interceptador
    servidor.
    \item[clientInterceptorConfig:] Configuração opcional do interceptador
    cliente.
    \item[policy:] Política de validação de credencial
  \end{description}
  Retorna \code{false} caso ocorra algum erro, e \code{true} caso contrário.
  ...
  \item \code{IRegistryService connectByLoginPassword(user, password}: Realiza uma tentativa de conexão com o barramento. Se a tentativa for bem sucedida, um \emph{stub} do Serviço de Registro é retornado. Caso contrário, o retorno é \emph{nil}. Uma exceção além das de CORBA pode ser lançada:
    \subitem \code{InvalidCredentialException}: Caso a credencial seja rejeitada ao tentar obter o Serviço de Registro.
  \item \code{IRegistryService connectByCertificate(name, privateKeyFile,  acsCertificateFile)}: Realiza uma tentativa de conexão com o barramento, utilizando o mecanismo de certificação para o processo de autenticação. Se a tentativa for bem sucedida, um \emph{stub} do Serviço de Registro é retornado. Caso contrário, o retorno é \emph{nil}. Uma exceção além das de CORBA pode ser lançada:
    \subitem \code{InvalidCredentialException}: Caso a credencial seja rejeitada ao tentar obter o Serviço de Registro.
  \item \code{IRegistryService connectByCredential(credential)}: Realiza uma tentativa de conexão ao barramento, a partir de uma credencial já existente. Se a tentativa for bem sucedida, um \emph{stub} do Serviço de Registro é retornado. Caso contrário, o retorno é \emph{nil}. Uma exceção além das de CORBA pode ser lançada:
  ...
  Os parâmetros são:
  \begin{description}
    \item[interfaceRepID] Identificador da interface do método.
    \item[method] Nome do método.
    \item[isInterceptable] Indica se o método deve ser interceptado ou não.
  \end{description}
...
local openbus = require "openbus.Openbus"
local HelloImpl = require "HelloImpl"
...
instance:addFacet("IHello", "IDL:demoidl/hello/IHello:1.0", HelloImpl())
\end{labeledcode}
